...
var
  s:string;
begin
  s:='select max(right(jl_nomor,4)) from tjual where jl_nomor like ' + quot(frmMenu.USERID+ NOMERATOR+'.'+FormatDateTime('yymm',cGetServerTime)+'.%');
  with xOpenQuery(s,frmMenu.conn) do
  begin
    try
      if Fields[0].AsString = '' then
         result:= frmMenu.USERID+NOMERATOR+'.'+FormatDateTime('yymm',cGetServerTime)+'.'+RightStr(IntToStr(10000+1),4)
      else
         result:= frmMenu.USERID+ NOMERATOR+'.'+FormatDateTime('yymm',cGetServerTime)+'.'+RightStr(IntToStr(10000+fields[0].AsInteger+1),4);
