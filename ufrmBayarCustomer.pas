...
uses
  ...
  dxSkinSummer2008, dxSkinValentine, dxSkinXmas2008Blue, cxTimeEdit;
...
type
  TfrmBayarCustomer = class(TForm)
    ...
    Button1: TButton;
    clKasir: TcxGridDBColumn;
    clJam: TcxGridDBColumn;
    clShift: TcxGridDBColumn;
    Label12: TLabel;
    ...
    procedure loaddataInvoice(akode : string; asalesman : string);
...
begin
  with TcxExtLookupHelper(cxLookupcustomer.Properties) do
    ...
    with TcxExtLookupHelper(cxLookupSalesman.Properties) do
    LoadFromCDS(CDSsalesman, 'Kode','Salesman',['Kode'],Self);
//    TcxExtLookupHelper(cxLookupSalesman.Properties).SetMultiPurposeLookup;
...
begin
  ...
  begin
    ...
    zAddField(FCDS, 'Kasir', ftString, False,20);
    zAddField(FCDS, 'Jam', ftdateTime, False);
    zAddField(FCDS, 'Shift', ftString, False,10);
    FCDS.CreateDataSet;
  end;
  Result := FCDS;
end;
...
function TfrmBayarCustomer.GetCDSsalesman: TClientDataset;
var s:String;
begin
  If not Assigned(FCDSsalesman) then
  begin
    S := 'select sls_nama as salesman, sls_kode Kode'
        +' from tsalesman order by sls_kode ';
...
procedure TfrmBayarCustomer.loaddataInvoice(akode : string; asalesman : string);
var
  s: string ;
  tsql : TSQLQuery;
  i:Integer;
begin
    s := ' select sls_nama Salesman,fp_nomor,fp_tanggal,fp_jthtempo,fp_taxamount,fp_amount,((fp_biayapr*(fp_amount-fp_taxamount)/100)+fp_biayarp) Biaya_Promosi,FP_DP, '
        ...
        + ' tfp_hdr.user_create as user_create, TIME(tfp_hdr.date_create) AS date_create,'
        + ' CASE '
        + ' WHEN TIME(tfp_hdr.date_create) BETWEEN ''06:00:00'' AND ''14:00:00'' THEN "1"'
        + ' WHEN TIME(tfp_hdr.date_create) BETWEEN ''13:00:00'' AND ''23:59:00'' THEN "2"'
        + ' ELSE "0" '
        + ' END AS shift '
        + ' from tfp_hdr '
...
//        + ' where '
//        + ' and fp_sls_kode = ' +  Quot(asalesman);
          ...
          begin
            ...
            while  not Eof do
              begin
                  ...
                  CDS.FieldByName('kasir').AsString       := fieldbyname('user_create').AsString;
                  CDS.FieldByName('jam').AsDateTime       := fieldbyname('date_create').AsDateTime;
                  CDS.FieldByName('shift').AsString      := fieldbyname('shift').AsString;
...
procedure TfrmBayarCustomer.cxLookupcustomerPropertiesEditValueChanged(
  Sender: TObject);
begin
  loaddataInvoice(cxLookupCustomer.EditValue, cxLookupSalesman.EditValue);
...
procedure TfrmBayarCustomer.Button1Click(Sender: TObject);
begin
...
procedure TfrmBayarCustomer.Button2Click(Sender: TObject);
begin
    CDS.Filter := cxGrdMain.DataController.Filter.FilterText;
    CDS.Filtered := True;
    CDS.First;
  ...
  while not CDS.Eof do
    begin
        If CDS.State <> dsEdit then CDS.Edit;
          CDS.FieldByName('pay').AsInteger :=0;
          cds.FieldByName('bayar').AsFloat := 0;
          CDS.Next;
