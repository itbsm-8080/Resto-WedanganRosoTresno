...
procedure TfrmStokIn.refreshdata;
begin
  FID:='';
  FLAGEDIT := FALSE;
  dttanggal.date:= date;
...
procedure TfrmStokIn.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmStokIn.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if Key = #13 then
      SelectNext(ActiveControl,True,True);
end;

procedure TfrmStokIn.simpandata;
var
  s:string;
begin

if FLAGEDIT then
  s:='update tstokin set '
    + ' sti_keterangan = ' + Quot(edtketerangan.Text)+ ','
    + ' sti_tanggal = ' + quotd(dttanggal.datetime)
    + ' where sti_nomor= ' + quot(FID) + ';'
else
begin
  s :=  ' insert into tstokin '
             + ' (sti_nomor,sti_tanggal,sti_keterangan) '
             + ' values ( '
             + Quot(getmaxkode) + ','
  ...
  xExecQuery(s,frmmenu.conn);
    ...
    s:='delete from tstokin_dtl where stid_sti_nomor='+Quot(FID)+';';
  xExecQuery(s,frmMenu.conn);
  ClientDataSet1.First;
  while not ClientDataSet1.Eof do
  begin
    s:='insert into tstokin_dtl (stid_sti_nomor,stid_item_id,stid_qty,stid_hpp) values ('
    + Quot(fid) + ','
...
procedure TfrmStokIn.deletedata;
var
  s:string;
begin

  s:='Delete from tstokin  '
    + ' where sti_nomor= ' + quot(FID) + ';' ;

  xExecQuery(s,frmmenu.conn);

  s:='Delete from tstokin_dtl  '
    + ' where stid_sti_nomor= ' + quot(FID) + ';' ;
...
procedure TfrmStokIn.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action := caFree;
   Release;
end;

function TfrmStokIn.getmaxkode:string;
var
  s:string;
begin
    s:='select max(right(sti_nomor,4)) from tstokin where sti_nomor like ' + quot('STI.'+FormatDateTime('yymm',cGetServerTime)+'.%');

  with xOpenQuery(s,frmMenu.conn) do
  begin
    try
      if Fields[0].AsString = '' then
         result:= 'STI.'+FormatDateTime('yymm',cGetServerTime)+'.'+RightStr(IntToStr(10000+1),4)
      else
         result:= 'STI.'+FormatDateTime('yymm',cGetServerTime)+'.'+RightStr(IntToStr(10000+fields[0].AsInteger+1),4);
...
procedure TfrmStokIn.cxButton1Click(Sender: TObject);
begin
    try
...
procedure TfrmStokIn.cxButton8Click(Sender: TObject);
begin
Release;
end;

procedure TfrmStokIn.cxButton2Click(Sender: TObject);
begin
   try
         if FormatDateTime('yyyy-mm-dd',dtTanggal.DateTime) < FormatDateTime('yyyy-mm-dd',cGetServerTime) then
        begin
...
procedure TfrmStokIn.listdata ;
begin
MyQuery1.Close;
MyQuery1.SQL.Text :='SELECT sti_nomor Nomor ,sti_tanggal Tanggal,sti_keterangan Keterangan  '
+ ' from tstokin where sti_tanggal = '+ quotd(dttanggalshow.date)
+ ' order by sti_nomor ';
MyQuery1.Open;


end;
procedure TfrmStokIn.FormShow(Sender: TObject);
begin
  dttanggalshow.date := Date;
refreshdata;
end;

procedure TfrmStokIn.cxGrid1DBTableView1DblClick(Sender: TObject);
begin
...
function TfrmStokIn.getidcategory(anam:string):integer;
var
  s:string;
  tsql:tsqlquery;
begin
...
procedure TfrmStokIn.cxButton4Click(Sender: TObject);
begin
      ClientDataSet1.Open;
 If ClientDataSet1.State <> dsEdit then ClientDataSet1.Edit;
...
procedure TfrmStokIn.loaddata(akode:string) ;
var
  s: string;
  tsql : TSQLQuery;
  bmp: TBitmap;
  ...
  Buffer      : Word;
begin
  s:= 'select stid_item_id,item_nama,stid_qty from tstokin_dtl INNER JOIN titem on item_id =stid_item_id '
  + ' where stid_sti_nomor = ' + Quot(akode) ;
...
procedure TfrmStokIn.cxGrid2DBTableView1Column3PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
       xcancel:= true;
...
procedure TfrmStokIn.cxButton5Click(Sender: TObject);
begin
listdata ;
end;

procedure TfrmStokIn.cxButton3Click(Sender: TObject);
VAR
  S:STRING;
  tsql:TSQLQuery;
begin
