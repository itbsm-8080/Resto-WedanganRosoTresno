...
type
  TfrmPembayaranLain = class(TForm)
    ...
    procedure edtNomorUmClickBtn(Sender: TObject);
    private
...
uses MAIN,uModuleConnection,uFrmbantuan,Ulib,uReport,ufrmListdp;
...
begin
   ...
   anomor := edtNomor.Text;
      s:= 'update tjurnal set  '
         ...
         + ' jur_referensiUM = '+ Quot(edtNomorUm.Text) + ','
   ...
   begin
     ...
     edtNomor.Text := anomor;
      s:= ' insert into tjurnal '
         + '( jur_no,jur_tanggal,jur_keterangan,jur_tipetransaksi,jur_referensium,date_create,user_create) values ( '
         + Quot(anomor) + ','
         + Quotd(dttanggal.DateTime)+','
         + Quot(edtKeterangan.Text) + ','
         + Quot('Pembayaran Lain')+','
         + Quot(edtNomorUm.Text) + ','
...
//   xExecQuery(s,frmMenu.conn);
  ...
  begin
   ...
   if FLAGEDIT then
   begin
       if ReferensiUmLama <> edtNomorUm.Text then
       begin
           s:='UPDATE tuangmuka SET um_isrealisasi=0 WHERE um_nomor ='+ Quot(ReferensiUmLama);
           tt.Append(s);
           s:='UPDATE tuangmuka SET um_isrealisasi=1 WHERE um_nomor ='+ Quot(edtNomorUm.Text);
           tt.Append(s);
       end;
   end
   else
...
begin
  ...
  s := ' select jur_no,jur_tanggal,jur_keterangan,jurd_debet,jurd_rek_kode,'
     + ' jurd_kredit,jurd_keterangan,jurd_cc_kode,rek_nama ,jurd_cus_kode,jur_referensiUM '
     + ' from tjurnal a'
       ...
       begin
         ...
         begin
           ...
           edtNamaAccount.Text := getnama('trekening','rek_kode',fieldbyname('jurd_rek_kode').AsString,'rek_nama');
            ...
            ReferensiUmLama := fieldbyname('jur_referensiUM').AsString;
            edtNomorUm.Text := fieldbyname('jur_referensiUM').AsString;
...
procedure TfrmPembayaranLain.clSupplierPropertiesButtonClick(
...
function TfrmPembayaranLain.cekkembar(akode:string;amemo:String;abaris:Integer):Boolean;
var
  s:string;
  i:Integer;
begin
...
procedure TfrmPembayaranLain.edtNomorUmClickBtn(Sender: TObject);
var
  s:string;
  tsql: TSQLQuery;
begin
  ...
  s:='select um_nomor,um_customer,um_nilai,um_jenisbayar from tuangmuka where um_isrealisasi=0 ';
  tsql := xOpenQuery(s,frmMenu.conn);
  with tsql do
  begin
    try
      if not eof then
      begin
        edtNomorUm.Text :=Fields[0].AsString;
        edtnilai.Text := FloatToStr(Fields[2].AsFloat);
      end;
     finally
       Free;
    end;
