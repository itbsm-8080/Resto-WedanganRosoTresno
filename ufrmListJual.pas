...
begin
      ...
      s:= ' SELECT MONTH(jl_tanggal) Bulan,YEAR(jl_tanggal) Tahun,jl_nomor Nomor,date_FORMAT(jl_tanggal ,"%Y-%m-%d") Tanggal,'
        + ' item_nama Nama,jld_varian Varian,jld_salestype SalesType ,jld_qty Qty,jld_disc Disc,(100-jld_disc)/100*jld_price*jld_qty Nilai,'
        + ' jld_user Served,item_category Category,timediff(jld_tgleksekusi ,jl_tanggal ) lama_eksekusi,jl_jeniscustomer JenisCustomer'
        + ' FROM tjual_dtl INNER JOIN titem'
        + ' ON item_id=jld_item'
        + ' INNER JOIN tjual ON jl_nomor=jld_jl_nomor'
        + ' where jld_status=2 and jl_tanggal between ' + QuotD(startdate.DateTime) + ' and ' + QuotD(enddate.DateTime);
...
//
        Skolom :='Bulan,Tahun,Nomor,Tanggal,Nama,Varian,SalesType,Qty,Disc,Nilai,Served,Category,Lama_eksekusi,JenisCustomer';
