...
uses
  ...
  cxGraphics, cxLookAndFeels, dxSkinsCore, dxSkinsDefaultPainters,StrUtils;
...
begin
  s :=  ' insert into tuser'
             + ' (user_kode,user_nama,user_password,user_id) '
             + ' values ( '
             + Quot(edtkode.Text) + ','
             + Quot(edtNama.Text) + ','
...
function Tfrmuser.getmaxid : string;
var
  s:string;
  tsql:TSQLQuery;
begin
  s:='select max(user_id) from tuser ';
  tsql := xOpenQuery(s,frmMenu.conn);
  with tsql do
  begin
    try
      if Fields[0].AsString = '' then
         ...
         Result :=RightStr(IntToStr(101+Fields[0].AsInteger),2);
     finally
       free;
     end;
  end;
...
begin
  ...
  i:= 0 ;
  s := 'select * from tmenu where men_modul=1 order by men_id';
  tsql := xOpenQuery(s, frmMenu.conn);
  with tsql do
  begin
   try
